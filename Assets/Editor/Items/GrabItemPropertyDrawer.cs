    ...
    public override VisualElement CreatePropertyGUI(SerializedProperty property)
    {
        ...
        if (database)
        {
            ...
            for (int i = 0; i < combinationArray.arraySize; i++)
            {
                ...
                if(potentialProp.boxedValue is CombinationRecipe recipe && recipe.InvolvesItem(idVal))
            ...
            for (int i = 0; i < processArray.arraySize; i++)
            {
                ...
                if (potentialProp.boxedValue is ProcessRecipe recipe && recipe.InvolvesItem(idVal))
                    filteredProps.Add(potentialProp);
            }
